...
ChartManager::~ChartManager()
{
  ...
  VERBOSE(0, "total cell count: " << cellcount  << endl);
  VERBOSE(0, "Hypo Count: " <<  ChartHypothesis::GetHypoCount() << endl);
  VERBOSE(0, "Skipped ratio: " << skipRatio << endl);
...
void ChartManager::ProcessSentence()
{
  ...
  clock_t start, end;  // measuring cube-pruning time
  start = clock();
  ...
  float elasped = (end - start);
  elasped /= (float)CLOCKS_PER_SEC;
  SetTotalTime(elasped);
