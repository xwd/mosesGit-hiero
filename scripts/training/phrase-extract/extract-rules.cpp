                       ...
                       , HoleCollection &holeColl, LabelIndex &labelIndex)
{
  ...
  // target
  if (options.pcfgScore) {
    double logPCFGScore = sentence.targetTree.GetNodes(startT,endT)[labelIndex[0]]->GetScore();
    rule.target = printTargetHieroPhrase(sentence, startT, endT, startS, endS, indexT, holeColl, labelIndex, logPCFGScore)
                + " [" + targetLabel + "]";
              ...
              , RuleExist &ruleExist)
{
  ...
  if (options.pcfgScore) {
    double logPCFGScore = sentence.targetTree.GetNodes(startT,endT)[0]->GetScore();
    rule.pcfgScore = std::exp(logPCFGScore);
  }
